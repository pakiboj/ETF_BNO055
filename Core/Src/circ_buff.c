...
void circular_buf_init(cbuf_handle_t cbuf, accel_sample_t* buffer, size_t size)
{
	assert(cbuf && buffer && size);
...
void circular_buf_put(cbuf_handle_t me, accel_sample_t data)
{
	assert(me && me->buffer);
...
int circular_buf_try_put(cbuf_handle_t me, accel_sample_t data)
{
	int r = -1;
...
int circular_buf_peek(cbuf_handle_t me, accel_sample_t* data, unsigned int look_ahead_counter)
{
	int r = -1;
	size_t pos;
